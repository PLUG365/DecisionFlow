...
### 決定: トランスポート入れ替えはやらない（2026-08-10）

**アプリ登録の実装負荷と管理コストが見合わない**という判断で見送った。残っていた未知（iframe 内で MSAL が通るか）は**確かめない**。

理由を残す。**本番は別テナントへ持っていく計画**（この文書の外、`UX_ROADMAP.md` の「別計画: 本番化(ALM)」）であり、アプリ登録は

- テナントごとに作り直す
- 委任アクセス許可 `CopilotStudio.Copilots.Invoke` に管理者同意を取り直す
- シークレット／証明書の更新を運用に抱える

一度きりの作業ではなく、環境が増えるたびに増える。カードをパネルで描くために払う額として大きすぎる。

**したがって、上の暫定策が恒久策になる。** パネルからは判断確定を行わず、判断タブへ案内する。この判断を覆すのは、Microsoft がコネクタ経由で activity を返すようになったときか、アプリ登録を別の理由で作ることになったとき。

## 設計: 対話UIを持たないチャネルで判断確定を止める（G12・2026-08-10 固定）

モデル依存の暫定策を、**トピック側の決定的なガード**に置き換える（暫定策も残す。多層防御）。

### チャネル ID の実値（トランスクリプトから確認）

`conversationtranscript.content` の activity に `channelId` が入っている。**publish の往復なしで読み取れる。**

| 経路 | `channelId` | `isDesignMode` |
| --- | --- | --- |
| Copilot Studio テストパネル | `pva-studio` | `true` |
| **Code Apps パネル（コネクタ経由）** | **`pva-autonomous`** | `false` |

`pva-autonomous` という名前のとおり、**対話UIを持たない経路**である。Adaptive Card は原理的に応答されない。文字列の当てずっぽうではなく、意味のあるガードになる。

### 許可する状態と操作 / 拒否する状態と操作（追加分）

| # | 状態 | カード発行 | 動作 |
| --- | --- | --- | --- |
| 0 | `System.Activity.ChannelId` が `pva-autonomous` | **しない** | `issue_decision_card` を呼ばずに案内して終了 |

**ブロックリストにする（許可リストにしない）。** 止めるのは `pva-autonomous` だけで、Teams / M365 Copilot / テストパネルは**一切通り道が変わらない**。既存の動く経路を壊す危険を構造的にゼロにする。将来チャネルが増えても、増えた側は素通りする（安全側ではないが、**今動いているものを壊さない**方を優先する）。

### 評価時点

トピックの**最初**。`validateApplicationContext` より前に置く。`issue_decision_card` は書き込みを伴うので、**その手前でなければ意味がない**。

### 守る不変条件（追加）

- **不変条件9**: `pva-autonomous` では `ds_decisioncard` が発行されない。既存カードが Superseded にならない

**このガードは fail-open である。** ブロックリストなので、`System.Activity.ChannelId` が空だったり未知の値を返したりすると条件が false になり、カードは発行される。これは「Teams を構造的に無傷に保つ」ために許可リストを避けた結果として**受け入れたトレードオフ**であって、見落としではない。将来チャネルを増やすときは、増えた側がここを素通りすることを前提に判断する。

### 担保

| 条件 | 自動テスト | 人間が実機で確認 |
| --- | --- | --- |
| ガードがトピックの先頭にある | `zdI.mcs.yml` を YAML でパースして順序を assert | — |
| 条件式とチャネル ID | 同上 | — |
| 実際に止まる | **守らない** | **パネルから判断確定を試し、案内が出て `ds_decisioncard` が増えないこと** → 済（切り分け2） |
| Teams が壊れていない | **守らない** | Teams で判断確定が従来どおり通ること → 済（2026-08-10 23:54 JST、公開版の `MsTeams` でカード表示から確定まで通った） |

**Submitted の申請が要る。** 2026-08-10 に UI から2件投入済みなので、今回は実測できる。

### 変更する範囲 / 変更しない範囲

| 変更する | 変更しない |
| --- | --- |
| `zdI.mcs.yml` にガードを1つ追加 | フロー定義、認可の判定式、6段ゲート |
| `tests/test_adaptive_card_decision_confirmation.py` | `postApplicationMessage`（カードを使わないので影響なし） |
| この文書 | アプリ側の暫定策（残す。多層防御） |

### 実機確認の結果（2026-08-10 22:58 JST・公開後・Submitted の申請で実施）

**害は消えた。** Submitted の申請（`6daa9933`、判断者は自分）を指定してパネルから判断確定を試した。**カードが発行される条件が揃った状態**での確認である。

| 確認 | 結果 |
| --- | --- |
| 応答 | 「Adaptive Card を表示できないため、この画面からは判断の確定を実行できません」＋申請詳細へのリンク |
| `ds_decisioncard` | ✅ **1行も増えていない**（最新は 20:28 のまま）。不変条件9 |

**ただしこの実測はガードを通っていなかった。** 下の切り分けを参照。

### 切り分け1: 上の実測で止めたのは Instructions 層だった（2026-08-10・トランスクリプト）

`conversationtranscript` の `042fd720`（22:58:09 開始）を読んだ。`DynamicPlanReceived.value.steps` は

```json
["ds_DecisionFlowAssistant.action.Get_ApplicationDetailUrl"]
```

の1件だけで、**`zdI` はプランに入っていない**。トピックに入っていないのだから `validateCardCapableChannel` も走っていない。会話開始は 13:58:09Z で publish（13:53:52Z）より後なので、ガードは生きていたが**到達しなかった**だけである。

つまりあの時点で害を止めていたのは**確率的な層（Instructions）だけ**で、決定的なバックストップは一度も実行されていなかった。

### 切り分け2: ガード単体で効くことを実証した（2026-08-10 23:45 JST）

Instructions 層を外してガードだけを露出させた。**変えたのはアプリ側の1点だけ**（`copilot-context.ts` の `[この画面の制約]` 行を落とす）。これで agent 側の「例外」条項の条件が成立しなくなり、プランナーは通常どおり `zdI` を選ぶ。**ローカルの dev サーバのみ。`push` も publish もしていない**（公開版こそが被検証対象なので触らない）。確認後ただちに revert 済み。

対象には **Draft の申請**（`1255749f`、判断者は未割当）を使った。こうすると結果が書き込みなしで一意に切り分かる。

| 起きたこと | 応答 | 意味 |
| --- | --- | --- |
| ガードが効く | `この画面では判断カードを表示できないため、ここでは判断を確定できません。Code Apps の判断タブから確定してください。` | `sendChannelCannotRenderCards` |
| ガードが素通り | `判断カードを発行できませんでした。提出済みの申請で、あなたが判断者に…` | `validateIssueResult`（Draft なので必ずこちら） |

**返ってきたのは前者の文面と一字一句同じだった。** よって:

- `validateCardCapableChannel` は実際に発火する
- **Power Fx の `System.Activity.ChannelId` は、activity JSON の `channelId` と同じ `pva-autonomous` を返す**。これが唯一の未検証点だった（トランスクリプトから読んだ値であって、Power Fx で評価した値ではなかった）
- `ds_decisioncard` は増えていない（最新は 20:28 のまま）。不変条件9

**これで G12 は「実際に効く」と言える。** 多層防御の2層が、それぞれ単独で効くことを別々に実証した。
