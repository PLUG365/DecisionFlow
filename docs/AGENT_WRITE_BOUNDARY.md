...
**ただし、どちらの層が止めたかは切り分けられていない。** 返ってきた文面がアプリの送った画面名（ダッシュボード）を引用しているため、Instructions 側の暫定策が先に効いてトピック自体が選ばれなかった可能性がある。その場合、この実測は `validateCardCapableChannel` を通っていない。

切り分けにはトランスクリプト（`conversationtranscript`、約30分遅れ）で `zdI` の `DialogTracing` に `validateCardCapableChannel` が出ているかを見る。**未実施。**

多層防御としては期待どおりなので急がないが、**「G12 のガードが実際に効いた」とはまだ言えない。**
