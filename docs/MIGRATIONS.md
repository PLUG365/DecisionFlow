...
## 2026-05-22 ds_decision 閲覧権限補正

Status: Applied 2026-05-22

### 背景（判断結果）

判断結果 `ds_decision` は判断者が作成する子レコードであり、申請者が親 `ds_application` を所有していても判断レコード自体の読み取り権限は自動では付与されない。また、`ds_Decider` は申請本体を全体閲覧できる一方、`ds_decision` が Basic Read のままだと自分が作成していない判断結果を読めなかった。

### 対象（判断結果）

- `ds_Decider` ロール: `ds_decision` の Read を Global に変更
- `Decision_OnCreated`: 判断作成時に当該 `ds_decision` を申請者と関係者へ `ReadAccess` 共有
- `Participant_PreDelete_RevokeAccess`: 関係者削除時に対象申請の既存 `ds_decision` 直接共有も解除
- 既存 `ds_decision` 15 件: `scripts/backfill_decision_read_access.py` で申請者・関係者へ `ReadAccess` を補正
...
py scripts/backfill_decision_read_access.py
```

### 適用結果（判断結果）

- `setup_security_roles.py`: `ds_Applicant` / `ds_Decider` / `ds_Admin` を再適用し、ソリューション含有検証 OK
- `deploy_notification_flows.py`: `Decision_OnCreated` を含む通知フロー 4 本を再作成・有効化・Flow API start 済み
- `deploy_access_flows.py`: `Participant_OnCreated_GrantAccess` を再作成・有効化。Code Apps 参照済みの `Participant_PreDelete_RevokeAccess` は workflow ID を維持したまま PATCH 更新・有効化
- `backfill_decision_read_access.py`: 対象判断レコード 15 件、GrantAccess 15/15 成功

### 検証（判断結果）

```powershell
py -m unittest tests.test_decision_access_backfill tests.test_access_flows tests.test_notification_flows tests.test_security_roles
```

### 注意（判断結果）

- `Participant_PreDelete_RevokeAccess` は Code Apps から参照される Power Apps V2 フローのため、削除再作成ではなく既存 workflow を更新する。workflow ID `8b9b0241-2f46-f111-bec6-7c1e525c11fc` は維持済み。
- `ds_Decider` の全体閲覧はロール権限で担保する。申請者・関係者の案件単位閲覧は `GrantAccess` による直接共有で担保する。

## 2026-05-22 子レコード共有 Cascade 補正

Status: Applied 2026-05-22

### 背景（子レコード共有）

関連資料 `ds_applicationresource`、会話履歴 `ds_message`、メンション `ds_mention` も申請に紐づく子レコードだが、実環境では親 `ds_application` から子テーブルへの Share/Unshare が `NoCascade` になっていた。関係者へ申請を共有しても、既存または後続の子レコード閲覧が安定しないため、リレーションの Cascade 設定と既存参加者の親申請共有を補正した。

### 対象（子レコード共有）

- `ds_participant_ds_application`: Share/Unshare = Cascade
- `ds_message_ds_application`: Share/Unshare = Cascade
- `ds_decision_ds_application`: Share/Unshare = Cascade
- `ds_applicationresource_ds_application`: Share/Unshare = Cascade
- `ds_mention_ds_message`: Share/Unshare = Cascade
- 既存 `ds_participant` から抽出した application/user 共有 8 件: `scripts/backfill_application_share_for_participants.py` で親申請の `ReadAccess,AppendToAccess` を再付与

### 実行手順（子レコード共有）

```powershell
py scripts/migrate_cascade_share.py
py scripts/backfill_application_share_for_participants.py
```

### 適用結果（子レコード共有）

- `migrate_cascade_share.py`: 対象 5 リレーションすべて `NoCascade` から `Cascade` へ更新
- `backfill_application_share_for_participants.py`: application/user 共有 8 件、GrantAccess 8/8 成功

### 検証（子レコード共有）

```powershell
py -m unittest tests.test_application_share_backfill tests.test_decision_access_backfill tests.test_access_flows tests.test_notification_flows tests.test_security_roles
```
...
- `DataverseService.createDecision()` は `ds_decision` 作成後、Code Apps の即時フィードバックのため `ds_application.ds_stage` も更新する
- `差し戻し` の場合は `ds_application.ds_submittedat` をクリアする
- 通知・最終整合・判断結果共有は `Decision_OnCreated` に委譲する
