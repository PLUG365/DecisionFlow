...
- 判断選択肢と判断理由を必須入力にする。
- `ds_decision` を作成する。
- 申請ステージを Decided に更新する。
- 差し戻しが選択された場合は申請ステージを Draft に戻し、申請者が修正・再提出できるようにする。
...
- 未実装/要調整: 関係者追加・削除ボタンなどの UI 表示制御は、設計上は申請者/判断者/共同判断者に限定する方針だが、現時点では Dataverse 権限を最終判定として扱う。必要に応じて Code Apps 側の表示制御を追加する。
