...
| `/masters`          | マスタ管理     | 全ユーザー               | カテゴリ別レギュレーションと判断選択肢を参照し、管理者/判断者はカテゴリを保守する |
...
| 管理             | マスタ管理（全ユーザー表示）           | `Settings2`                               |

ヘッダー表示:

- タイトル: `DecisionFlow`
- サブタイトル: `判断を前に進めるワークスペース`
...
- カテゴリ選択後はフォーム内にレギュレーション本文を展開せず、情報ボタンから別モーダルで確認する。カテゴリ選択によって申請作成モーダルの高さが変化しないようにする。
- 提出済みにする場合は判断者を必須にし、未選択なら保存前にエラー表示する。
- AI事前確認または提出時AI判断の実行中は、画面全体に待機オーバーレイを表示し、処理中であることとそのまま待つ案内を出す。待機オーバーレイは Dialog/FormModal より前面に出る最上位レイヤーで表示する。
- AI関連の待機文言・完了ダイアログ・詳細導線には、業務アプリとして控えめな範囲で `✨` / `🔎` / `☕` などの絵文字を使い、処理中でも親しみを感じられる表現にする。
- 下書きAI事前確認の完了後もAIコメントダイアログを表示する。下書き時は「閉じる」と「詳細はこちら」だけを表示し、提出時は同じダイアログで「本提出」「下書き維持」「詳細はこちら」を表示する。
- AIコメントダイアログの「詳細はこちら」から申請詳細の判断タブへ遷移し、AI判断カードの詳細を確認できるようにする。
...
- 承認/却下は申請ステージを Decided に更新し、差し戻しは Draft に戻して `ds_submittedat` をクリアする。通知・最終整合・判断結果共有は `Decision_OnCreated` が担当する。
...
- `AI判断更新` ボタンは判断済みではない申請で使用できる。押下時に Power Apps V2 フロー `Application_GenerateAiDecision` を呼び出し、判断タブ上のAI判断カードを更新する。判断タブ内に結果表示があるため、申請一覧の提出確認で使うAIコメントダイアログは表示しない。
- `AI判断更新` ボタン押下中は最前面の待機オーバーレイを表示し、利用者が連打や画面遷移をしないように処理中状態を明示する。
...
- カテゴリ / 判断選択肢の `Tabs`。
- カテゴリは簡易編集用 `InlineEditTable` で表示し、`ds_Admin` / `ds_Decider` のみ追加・編集・削除に対応。
...
- `useIsAdmin()` / `useIsDecider()` Hook と `canEditMasterData()` で編集可否を判定する。
- サイドバーと `/masters` ルートは全ユーザーに表示し、申請者はカテゴリ別レギュレーションと判断選択肢を読取り専用で参照する。
...
- カテゴリが空の場合だけ、初期カテゴリ（顧客案件 / 部内案件 / 課内案件 / 他部署案件 / 事務処理）と各カテゴリの初期レギュレーションを作成する。
- 標準カテゴリ名が既に存在し、`ds_regulationtext` が空の場合は、既存カテゴリを作り直さず初期レギュレーションだけ補完する。既に入力済みのレギュレーションは上書きしない。
...
- 実装済みフォーム/操作:
  ...
  - [x] 関連資料リンク追加
  - [x] 関連資料リンクの確認付き削除
  - [x] カテゴリの追加・更新・削除（ds_Admin / ds_Decider のみ編集可能。Applicant は読取り専用）
...
- `Participant_PreDelete_RevokeAccess` は `npx power-apps add-flow` で SDK 生成済み。アクセス制御フローを UI 編集しやすい Compose payload + 接続参照版に更新し、2026-05-22 の判断結果共有解除対応では Code Apps 参照済み workflow ID を維持したまま PATCH 更新済み。サービス層は [src/services/dataverse-service.ts](../src/services/dataverse-service.ts) で `Run` の `ok` が `true`、または共有解除不要と判定できる既知エラーの場合のみ削除する。
...
- 未実装/要調整: 関係者追加・削除ボタンなどの UI 表示制御は、現時点では Dataverse 権限を最終判定として扱う。必要に応じて Code Apps 側の表示制御を追加する。マスタ管理は全ユーザーに表示し、編集操作は `canEditMasterData()` で ds_Admin / ds_Decider のみに制御済み。
