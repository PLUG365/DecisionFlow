...
30. **アイコンは `standard` スキルの `references/icon-creation.md` に従い生成・登録**。エージェント=PNG 3サイズ（240, 192, 32）で `bots.iconbase64` + Teams マニフェスト。テーブル=SVG WebResource + `IconVectorName`。詳細は `.github/skills/standard/references/icon-creation.md`
...
78. **Code Apps 設計**: `code-apps` スキルの `references/design-system.md` を読み、画面構成・コンポーネント選定・Lookup 名前解決パターンを設計。ユーザー承認後に `code-apps` スキルで実装
...
2. **`architecture` スキルを読み込み、アーキテクチャ判断を行う**: Copilot Studio / Power Automate / Code Apps / AI Builder の使い分けを決定し、統合パターンを選定する
...
1. `code-apps` スキルの `references/design-system.md` を読み込む
...
1. `copilot-studio` スキルの `references/trigger.md` を読み込む
2. トリガー設計書を作成し提示:
