...
開発端末に公式 Power Automate プラグイン（FlowAgent）が導入されている場合、
**新規フローの設計補助・コネクタ探索・定義検証・失敗診断**は FlowAgent を第一候補にする。
実際の利用手順・PAC MCP / Dataverse MCP の設定は
[AI 開発ツールの利用規約](../../../docs/AI_DEVELOPMENT_TOOLING.md) を参照する。

開発は「探索・試作」と「採用・運用」の二レーンで行う。スクリプトを先に書くことは要求しない。

- **探索・試作**: FlowAgent で開発環境の新規・一時的なフローを停止状態で作成、編集、検証してよい。
  `validate_flow` と `preflight_flow` を通すが、既存の管理対象を置換・削除・公開しない。
- **採用・運用**: 共有、継続保守、複数環境展開、業務影響を伴うフローは、Python スクリプトまたは
  Solution 成果物の Git 管理された正本へ定義・接続参照・検証結果を昇格させ、レビュー後に適用する。

ただし、DecisionFlow の既存フローとデプロイスクリプトは採用・運用の正本である。

- 既存フローを FlowAgent で削除、置換、公開しない。提案された定義はレビュー後に、
  再現可能な Python スクリプトまたは Solution 成果物へ反映する。
- FlowAgent で作る探索用の新規フローは、開発環境で**停止状態**に限る。
  `validate_flow` と `preflight_flow` を通し、フロー設計・接続参照・最小スモークテストを
  レビューしてから、明示承認を受けて有効化する。
- 接続は既存のソリューション接続参照を使う。接続を API やエージェントで自動作成しない。
- Dataverse MCP はメタデータと読取りクエリを既定とする。書込み、削除、ロール、環境設定、
  Solution の import / publish は、対象と影響を示した明示承認が必要である。
...
### 接続検索は admin scope API を優先し、OAuth 接続を選ぶ

```
検証結果（2026-05-04 DecisionFlow）:
    ❌ /providers/Microsoft.PowerApps/apis/{connector}/connections だけでは環境内接続を取れないことがある
    ❌ displayName が "Microsoft Dataverse" で connectionParametersSet.values が空の接続を使うと、
            有効化時に ConnectionMissingAuthenticatedUserObjectId で失敗することがある

    ✅ /providers/Microsoft.PowerApps/scopes/admin/environments/{env-id}/connections を使い、
            apiId の末尾が対象 connector と一致する接続だけを候補にする
    ✅ statuses が Connected で、connectionParametersSet.values に token:grantType を持つ OAuth 接続を優先する
```
