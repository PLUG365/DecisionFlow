...
| 項目                       | 内容                                                                   |
| -------------------------- | ---------------------------------------------------------------------- |
| ベースロール               | 既定は Basic User のコピー。**複数テナントへ移送するならコピーしない**（下記） |
...
### ロールの土台をどうするか（**運用前提で変わる。既定はコピー**）

**先に決めること: 利用者にカスタムロールを1つだけ割り当てるのか、`Basic User` と併せて割り当てるのか。**
ここで土台の作り方が変わる。Dataverse のロールは加算式なので、両方割り当てるなら土台は要らない。

| 前提 | 土台 | 代償 |
| --- | --- | --- |
| **カスタムロール1つだけを割り当てる**（既定） | `Basic User` をコピーして上乗せ | ロールが約 480 + カスタム件になり、**環境をまたいで移送できない**（下記） |
| **`Basic User` と併せて割り当てる** | コピーしない。カスタムテーブル権限だけ | 割り当てが2ロールになる。**利用者が `Basic User` を持っていないとアプリが動かない** |

```
★★★ 1ロール運用ならコピーは必須 ★★★

ゼロから作ったロールを単独で割り当てると、プラットフォーム標準権限（メール送信・
ダッシュボード表示・検索・ユーザー設定・ビュー切替等の約 480 権限）が無いため、
ユーザーはアプリを正常に使えない。
...
#### ただし、コピーしたロールは別環境へ移送できないことがある（実測 2026-08-12）

**コピーは「その環境の `Basic User`」を取り込む。深度の可否は環境ごとに違うことがあり、
移送先が受け付けない深度が混ざるとソリューション import が丸ごと失敗する。**

実測した例:

```text
The privilege prvDeleteUserSettings with id = 00bcb198-f3d8-4451-98a1-36642a084b8f
can't have depth = Basic, RoleId = ...
```

| 権限 | 環境A `canbebasic` | 環境B `canbebasic` |
| --- | --- | --- |
| `prvDeleteUserSettings` | true | **false** |
| `prvWriteUserSettings` | true | **false** |

環境Aの `Basic User` がこの2件を Basic で持っていたため、コピーしたロールも Basic を持ち、
環境Bへの import が拒否された。**同じソリューションが、片方の環境でだけ入らない。**
ロールの権限が約 480 件あるぶん、どれか1件が非互換なだけで詰まる。

**判断の目安**:

- 単一環境で完結する、または移送先ごとにこのスクリプトを流せる → **コピーでよい**
- ソリューションを複数テナントへ配布する（ALM を回す）→ **コピーしない**。
  ロールはカスタムテーブル分だけにして、`Basic User` の併用を配布手順に明記する

コピーしない構成を選ぶ場合、**配布手順書に「`Basic User` を併せて割り当てる」を必ず書くこと。**
書かないと、手順どおりに新環境を立てたのに動かないユーザーができる。
