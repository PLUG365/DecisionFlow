...
1. Code Apps で案件ステージが `Decision_OnCreated` により反映されることを確認する
...
2026-05-19 時点では、環境スクリプト反映と Power Automate ツールフロー 2 本の作成まで完了済み。Copilot Studio UI の専用 Topic / ツール wiring と Teams 実機確認は未完了。

- まず T061 では、正常系、入力必須、`ds_decision` 作成、`ds_decisioncard` 消費、`Decision_OnCreated` によるステージ整合を確認する
- 未割り当てユーザー拒否、古いカード拒否、再利用カード拒否の詳細 validation は US2 実装後に合格判定する
- Code Apps の 500ms polling / 楽観ステージ表示は US3 実装後に合格判定する
...
- 判断後整合フローを無効化する場合は、Code Apps 側が `ds_application` を直接更新しない設計へ移行済みであるため、判断後ステージ反映が止まる。復旧まで `Decision_OnCreated` を再有効化するか、暫定補正手順を `docs/PLAN.md` に記録する
